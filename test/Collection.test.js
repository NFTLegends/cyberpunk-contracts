...
contract('Collection : ERC721', function([deployer, others]) {
  ...
  context('Calculator test', function() {
    ...
    context('addSaleStage()', function() {
      ...
      it('reverts when trying to add stage with wrong endTokens', async function() {
        await expectRevert(this.token.addSaleStage(0, 100), 'addSaleStage: first stage endTokens must be non-zero');
        await this.token.addSaleStage(1, 100);
    ...
    context('buy()', function() {
      ...
      beforeEach(async function() {
        await this.token.setVault(this.vault.address, {
    ...
    context('mintMultiple()', function() {
      ...
      it('deployer has MINTER_ROLE', async function() {
        await this.token.mintMultiple(this.mintMultipleSetter.address, 1, {
          from: this.owner.address,
        });
        expect(await this.token.totalSupply()).to.be.bignumber.equal('1');
      });
      ...
      it('token purchase', async function() {
        await this.token.mintMultiple(this.mintMultipleSetter.address, 1, {
          from: this.mintMultipleSetter.address,
        });
        expect(await this.token.totalSupply()).to.be.bignumber.equal('1');

        await this.token.mintMultiple(this.mintMultipleSetter.address, 2, {
          from: this.mintMultipleSetter.address,
        });
        expect(await this.token.totalSupply()).to.be.bignumber.equal('3');
      });
      ...
      it('reverts when trying to buy after sale end', async function() {
        await this.token.mintMultiple(this.mintMultipleSetter.address, 10, {
          from: this.mintMultipleSetter.address,
        });
        expect(await this.token.totalSupply()).to.be.bignumber.equal('10');

        await expectRevert(
          this.token.mintMultiple(this.mintMultipleSetter.address, 10, {
            from: this.mintMultipleSetter.address,
          }),
      ...
      it('reverts when trying to buy 0 nft', async function() {
        await expectRevert(
          this.token.mintMultiple(this.mintMultipleSetter.address, 0, {
            from: this.mintMultipleSetter.address,
          }),
      ...
      it('reverts when trying to buy nfts that exceeds totalSupply', async function() {
        await expectRevert(
          this.token.mintMultiple(this.mintMultipleSetter.address, 20, {
            from: this.mintMultipleSetter.address,
          }),
      ...
      it('without Admin role, function should revert', async function() {
        await expectRevert(
          this.token.mintMultiple(this.owner.address, 1, {
    ...
    context('getBatchByToken()', function() {
      ...
      it('revertsd when tokenId greater then last token id in batches array', async function() {
        await this.token.addBatch(10, 'testBaseURI', 12, {
          from: this.batchManagerAddress.address,
        });
        await this.token.addBatch(30, 'testBaseURI2', 7, {
          from: this.batchManagerAddress.address,
        });
        await expectRevert(
      ...
      it('works when return right baseURI (second batch)', async function() {
        await this.token.addBatch(10, 'testBaseURI', 84, {
          from: this.batchManagerAddress.address,
        });
    ...
    context('addBatch()', function() {
      ...
      it('reverts without BATCH_MANAGER_ROLE', async function() {
        await expectRevert(
          this.token.addBatch(0, 'ipfs://QmSQENpQaQ9JLJRTXxDGR9zwKzyXxkYsk5KSB3YsGQu78a', 59, {
            from: this.noRoleAddress.address,
          }),
          'VM Exception while processing transaction: revert AccessControl',
        );
      ...
      it('reverts when first batch endTokens is zero', async function() {
        await expectRevert(
          this.token.addBatch(0, 'testBaseURI', 40, {
            from: this.batchManagerAddress.address,
          }),
      ...
      it('reverts when batchEndId greater than the endId of the last batch', async function() {
        await this.token.addBatch(10, 'testBaseURI', 67, {
          from: this.batchManagerAddress.address,
        });
        await expectRevert(
          this.token.addBatch(10, 'testBaseURI2', 91, {
            from: this.batchManagerAddress.address,
          }),
    ...
    context('deleteBatch()', function() {
      beforeEach(async function() {
        this.batchManagerRole = await this.token.BATCH_MANAGER_ROLE();
        await this.token.grantRole(this.batchManagerRole, this.batchManagerAddress.address);
        await this.token.addBatch(10, 'ipfs://QmXkzp3EvcqnTPsHstwc89C91S64YAbBQotrgq8atLzHT3', 69, {
          from: this.batchManagerAddress.address,
        });
      });
      ...
      it('reverts then delete batch without BATCH_MANAGER_ROLE', async function() {
        await expectRevert(
          this.token.deleteBatch(0, { from: this.noRoleAddress.address }),
          'VM Exception while processing transaction: revert AccessControl',
        );
      });
      ...
      it('works when index out of batches length', async function() {
        await expectRevert(
          this.token.deleteBatch(2, { from: this.batchManagerAddress.address }),
    ...
    context('tokenURI()', function() {
      ...
      beforeEach(async function() {
        this.batchManagerRole = await this.token.BATCH_MANAGER_ROLE();
        await this.token.grantRole(this.batchManagerRole, this.batchManagerAddress.address);
        await this.token.addBatch(9, 'ipfs://QmXkzp3EvcqnTPsHstwc89C91S64YAbBQotrgq8atLzHT3', 67, {
          from: this.batchManagerAddress.address,
        });
      });
      ...
      it('works when tokenURI is returned (few tokens)', async function() {
        await this.token.addBatch(19, 'ipfs://QmSQENpQaQ9JLJRTXxDGR9zwKzyXxkYsk5KSB3YsGQu78a', 71, {
          from: this.batchManagerAddress.address,
        });
        ans = await this.token.tokenURI(17);
        expect(ans).equal('ipfs://QmSQENpQaQ9JLJRTXxDGR9zwKzyXxkYsk5KSB3YsGQu78a/17.json');
      });

      it('works when tokenURI is returned (10 batches)', async function() {
        await this.token.addBatch(19, 'ipfs://QmWADyUFUzVfcXPBfxVNceKExb4Veitt5Cy5ynMLVoeTF7', 11, {
          from: this.batchManagerAddress.address,
        });
        await this.token.addBatch(29, 'ipfs://QmRhtAr9cexKezrd17iTL8DLm9ue8JkQcKwU59Ki7ntZSM', 12, {
          from: this.batchManagerAddress.address,
        });
        await this.token.addBatch(39, 'ipfs://QmZxqcENyJVVywUj6kNpiGhY7upXsMWQTAB67fEqPVUmVx', 13, {
          from: this.batchManagerAddress.address,
        });
        await this.token.addBatch(49, 'ipfs://QmZAdGuo5DTjqEXWL9Xyi3cooeScwTgTaZQiGcWAwRyjX5', 14, {
          from: this.batchManagerAddress.address,
        });
        await this.token.addBatch(59, 'ipfs://QmeTbDTpz5DZNzbM3a6TKK7srF8rAYFGxrqdiNQNqk57wR', 15, {
          from: this.batchManagerAddress.address,
        });
        await this.token.addBatch(69, 'ipfs://QmNcQmj1AbcLz16YqfRRJtWfnSVsAuBJnkzFifamHHbzqs', 16, {
          from: this.batchManagerAddress.address,
        });
        await this.token.addBatch(79, 'ipfs://Qmcq57emNzG99Efhi9EFT7JKMhWEAjxHU3PYRTYhDRsC6S', 17, {
          from: this.batchManagerAddress.address,
        });
        await this.token.addBatch(89, 'ipfs://QmZXWVccpzk8vQAcd4yRndPMdNpaDWEGXdMeFBYFe8xrpY', 18, {
          from: this.batchManagerAddress.address,
        });
        await this.token.addBatch(99, 'ipfs://QmeCWYEJjg962DhAJGGqerL25dtZ98GwhfwMequ4jmixX2', 19, {
          from: this.batchManagerAddress.address,
        });
        ans = await this.token.tokenURI(0);
        expect(ans).equal('ipfs://QmXkzp3EvcqnTPsHstwc89C91S64YAbBQotrgq8atLzHT3/0.json');
        ans = await this.token.tokenURI(10);
        expect(ans).equal('ipfs://QmWADyUFUzVfcXPBfxVNceKExb4Veitt5Cy5ynMLVoeTF7/10.json');
        ans = await this.token.tokenURI(20);
        expect(ans).equal('ipfs://QmRhtAr9cexKezrd17iTL8DLm9ue8JkQcKwU59Ki7ntZSM/20.json');
        ans = await this.token.tokenURI(30);
        expect(ans).equal('ipfs://QmZxqcENyJVVywUj6kNpiGhY7upXsMWQTAB67fEqPVUmVx/30.json');
        ans = await this.token.tokenURI(40);
        expect(ans).equal('ipfs://QmZAdGuo5DTjqEXWL9Xyi3cooeScwTgTaZQiGcWAwRyjX5/40.json');
        ans = await this.token.tokenURI(50);
        expect(ans).equal('ipfs://QmeTbDTpz5DZNzbM3a6TKK7srF8rAYFGxrqdiNQNqk57wR/50.json');
        ans = await this.token.tokenURI(60);
        expect(ans).equal('ipfs://QmNcQmj1AbcLz16YqfRRJtWfnSVsAuBJnkzFifamHHbzqs/60.json');
        ans = await this.token.tokenURI(70);
        expect(ans).equal('ipfs://Qmcq57emNzG99Efhi9EFT7JKMhWEAjxHU3PYRTYhDRsC6S/70.json');
        ans = await this.token.tokenURI(80);
        expect(ans).equal('ipfs://QmZXWVccpzk8vQAcd4yRndPMdNpaDWEGXdMeFBYFe8xrpY/80.json');
        ans = await this.token.tokenURI(90);
        expect(ans).equal('ipfs://QmeCWYEJjg962DhAJGGqerL25dtZ98GwhfwMequ4jmixX2/90.json');
      });

      it('works when tokenURI is returned (few tokens in few batches)', async function() {
        await this.token.addBatch(19, 'ipfs://QmWADyUFUzVfcXPBfxVNceKExb4Veitt5Cy5ynMLVoeTF7', 2, {
          from: this.batchManagerAddress.address,
        });
        ...
        expect(ans).equal('ipfs://QmXkzp3EvcqnTPsHstwc89C91S64YAbBQotrgq8atLzHT3/1.json');
        ans = await this.token.tokenURI(2);
        expect(ans).equal('ipfs://QmXkzp3EvcqnTPsHstwc89C91S64YAbBQotrgq8atLzHT3/2.json');
        ans = await this.token.tokenURI(10);
        expect(ans).equal('ipfs://QmWADyUFUzVfcXPBfxVNceKExb4Veitt5Cy5ynMLVoeTF7/10.json');
        ans = await this.token.tokenURI(11);
        expect(ans).equal('ipfs://QmWADyUFUzVfcXPBfxVNceKExb4Veitt5Cy5ynMLVoeTF7/11.json');
        ans = await this.token.tokenURI(12);
        expect(ans).equal('ipfs://QmWADyUFUzVfcXPBfxVNceKExb4Veitt5Cy5ynMLVoeTF7/12.json');
      });
    });
  });
  ...
  describe('setDefaultRarity', function() {
    ...
    it('DEFAULT_RARITY_SETTER_ROLE can set default rarity', async function() {
      await this.token.setDefaultRarity(defRarity, {
        from: this.setterDefaultRarityAddress.address,
      });
      await this.token.addBatch(10, 'testBaseURI', 99, {
        from: this.batchManagerAddress.address,
      });
    ...
    it('reverts without DEFAULT_RARITY_SETTER_ROLE', async function() {
      await expectRevert(
        this.token.setDefaultRarity(defRarity, {
          from: this.noRoleAddress.address,
        }),
        'VM Exception while processing transaction: revert AccessControl',
      );
  ...
  describe('without defaultUri role, setDefaultUri should revert', function() {
    it('exception', async function() {
      await expectRevert(
        this.token.setDefaultUri('https://nftlegends.io/', {
          from: this.noRoleAddress.address,
        }),
        'VM Exception while processing transaction: revert AccessControl',
      );
  ...
  describe('without Admin role, start and stop should revert', function() {
    it('exception', async function() {
      await this.token.setVault(this.owner.address, {
        from: this.owner.address,
      });
      await expectRevert(
  ...
  describe('setName', function() {
    ...
    it('deployer has NAME_SETTER_ROLE', async function() {
      expectEvent(await this.token.setName(0, newName, { from: this.owner.address }), 'NameChange', {
        index: '0',
        newName: newName,
      });
    ...
    it('NAME_SETTER_ROLE can change the name', async function() {
      expectEvent(
        await this.token.setName(0, newName, {
          from: this.nameSetterAddress.address,
        }),
        'NameChange',
        {
          index: '0',
  ...
  describe('setSkill', function() {
    ...
    it('deployer has SKILL_SETTER_ROLE', async function() {
      expectEvent(await this.token.setSkill(0, newSkill, { from: this.owner.address }), 'SkillChange', {
        index: '0',
        newSkill: newSkill,
      });
    ...
    it('SKILL_SETTER_ROLE can change the skill', async function() {
      expectEvent(
        await this.token.setSkill(0, newSkill, {
          from: this.skillSetterAddress.address,
        }),
        'SkillChange',
        {
          index: '0',
  ...
  describe('setDna', function() {
    const newDna = new BN(2234);
    beforeEach(async function() {
      this.token = await ERC721Mock.new();
      await this.token.mint(this.owner.address, 1);
    });
    it('deployer has DNA_SETTER_ROLE and able to set DNA', async function() {
      expect(await this.token.getDna(0)).to.be.bignumber.equal('0');
      expectEvent(await this.token.setDna(0, newDna, { from: this.owner.address }), 'DnaChange', {
        index: '0',
        newDna: newDna,
      });
      expect(await this.token.getDna(0)).to.be.bignumber.equal(newDna);
    });
    it('reverts without DNA_SETTER_ROLE', async function() {
      await expectRevert(
        this.token.setDna(0, newDna, { from: this.noRoleAddress.address }),
        'VM Exception while processing transaction: revert AccessControl',
      );
    });
  });
  ...
  describe('setMaxPurchaseSize', function() {
    ...
    it('deployer has MAX_PURCHASE_SIZE_SETTER_ROLE', async function() {
      await this.token.setMaxPurchaseSize(newPurchaseSize, {
        from: this.owner.address,
      });
    ...
    it('reverts without MAX_PURCHASE_SIZE_SETTER_ROLE', async function() {
      await expectRevert(
        this.token.setMaxPurchaseSize(newPurchaseSize, {
          from: this.noRoleAddress.address,
        }),
        'VM Exception while processing transaction: revert AccessControl',
      );
